...
export type BlogStatus = "DRAFT" | "PENDING" | "APPROVED" | "PUBLISHED" | "REJECTED";
...
const resolveDocType = (item: BlogItem): DocType => {
    ...
    if (categoryMatch.includes("blog") || categoryMatch.includes("article") || item.coverImage?.url) {
        return "BLOG";
    }
...
export default function ReviewQueue() {
    ...
    const fetchQueueData = async () => {
        ...
        setError(null);
        try {
            const response = await apiClient.get<GetBlogsResponse>("/blog/getall");
            ...
            setError(err?.response?.data?.message || "Internal server error fetching queue.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const pendingResearchCount = useMemo(() => {
        return blogs.filter((b) => {
            ...
            return resolveDocType(b) === "RESEARCH" && (status === "DRAFT" || status === "PENDING");
        }).length;
    }, [blogs]);
    ...
    const pendingBlogCount = useMemo(() => {
        return blogs.filter((b) => {
            const status = b.status?.toUpperCase() || "";
            return resolveDocType(b) === "BLOG" && (status === "DRAFT" || status === "PENDING");
        }).length;
    }, [blogs]);
    ...
    const activeDataset = activeTab === "ALL" ? filteredBlogs : activeTab === "RESEARCH" ? researchQueue : blogQueue;
    const totalPages = Math.max(1, Math.ceil(activeDataset.length / itemsPerPage));
    ...
    const paginatedResearch = activeTab === "ALL"
        ? paginatedDataset.filter(b => resolveDocType(b) === "RESEARCH")
        : activeTab === "RESEARCH" ? paginatedDataset : [];

    const paginatedBlog = activeTab === "ALL"
        ? paginatedDataset.filter(b => resolveDocType(b) === "BLOG")
        : activeTab === "BLOG" ? paginatedDataset : [];
                            ...
                            <section className="space-y-4">
                                <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                                    ...
                                    <span className="rounded-full bg-amber-100/80 px-3 py-1 text-xs font-bold text-amber-800 border border-amber-200">
                                        {pendingResearchCount} Pending System-Wide
                                    </span>
                                </div>

                                {paginatedResearch.length === 0 ? (
                                    activeTab === "ALL" && paginatedBlog.length > 0 ? null : (
                                    ...
                                    <div className="space-y-4">
                                        {paginatedResearch.map((item) => (
                                            <ResearchQueueCard
                                                key={item._id}
                                                item={item}
                                                onOpenReview={() => navigate(`/editor/review/${item.slug}`)}
                                            />
                                        ))}
                                    </div>
                                )}
                        ...
                        {(activeTab === "ALL" || activeTab === "BLOG") && (
                            <section className="space-y-4">
                                ...
                                {paginatedBlog.length === 0 ? (
                                    activeTab === "ALL" && paginatedResearch.length > 0 ? null : (
                        ...
                        {activeDataset.length > 0 && (
                            <div className="mt-8 flex items-center justify-between rounded-xl bg-white p-4 shadow-sm border border-slate-200">
                                <p className="text-xs text-slate-500 font-medium">
                                    Showing <span className="font-bold text-slate-800">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-bold text-slate-800">{Math.min(currentPage * itemsPerPage, activeDataset.length)}</span> of <span className="font-bold text-slate-800">{activeDataset.length}</span> published entries
                                </p>
            ...
            <footer className="mt-12 border-t border-slate-200 bg-white py-3 px-4 text-center text-xs text-slate-500">
                <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                        ...
                        <span>
                            System Metrics Evaluated: Research Drafts/Pending ({pendingResearchCount}) • Blog Drafts/Pending ({pendingBlogCount})
                        </span>
                    </div>
                    <span className="font-mono text-[11px] text-slate-400">Schema: Client Visibility Locked to PUBLISHED</span>
                </div>
            </footer>
        </div>
    );
                ...
                <div className="flex flex-col justify-between space-y-3 lg:col-span-3 lg:border-l lg:border-slate-100 lg:pl-6">
                    <div className="flex items-center justify-between">
                        <div className="text-[11px] font-semibold text-slate-500">
                            Attachments ({pdfCount} PDFs{hasCsv ? ", 1 CSV" : ""})
                        </div>
                    ...
                    <div className="space-y-1.5">
                        {item.pdfs?.slice(0, 2).map((pdf, idx) => (
                            <div key={idx} className="flex items-center justify-between rounded bg-slate-50 px-2 py-1 text-[11px] text-slate-600">
                                <span className="flex items-center gap-1.5 truncate">
                                    <FileText className="h-3 w-3 text-red-500 shrink-0" />
                                    <span className="truncate">{pdf.originalName || `attachment_${idx + 1}.pdf`}</span>
                                </span>
                            </div>
                        ))}
                        {hasCsv && (
                            <div className="flex items-center justify-between rounded bg-emerald-50/60 px-2 py-1 text-[11px] text-emerald-800">
                                ...
                                <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-700">CSV Attached</span>
                            </div>
                        )}
                        {!pdfCount && !hasCsv && (
    ...
    return (
        <div className="group rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs transition-all hover:border-slate-300 hover:shadow-md">
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-12 lg:items-center">
                ...
                <div className="flex flex-col justify-between space-y-3 lg:col-span-3 lg:border-l lg:border-slate-100 lg:pl-6">
                    <div className="flex items-center justify-between">
                        <span className="text-[11px] font-semibold text-slate-500">Media & Artifacts</span>
                        <StatusBadge status={item.status} />
                    </div>

                    <div className="space-y-1 text-[11px] text-slate-600">
                        <div className="flex justify-between items-center rounded bg-slate-50 px-2 py-1">
                            <span className="truncate">{item.coverImage?.url ? "cover_image.webp" : "No Cover"}</span>
                            <span className="font-bold text-emerald-600">Verified</span>
                        </div>
                        <div className="flex justify-between items-center rounded bg-slate-50 px-2 py-1">
                            <span>PDF / CSV</span>
                            <span className="text-slate-400">
                                {item.pdfs?.length || item.csv?.url ? "Attached" : "None"}
                            </span>
                        </div>
                    </div>
...
function EmptyQueueState({ message }: { message: string }) {
    return (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white p-8 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">Try resetting search filters or category toggles.</p>
        </div>
    );
}
